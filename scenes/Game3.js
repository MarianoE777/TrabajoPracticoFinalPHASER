export default class Game3 extends Phaser.Scene {
  ...
  init(data) {
    ...
    this.tiempoRestante = 20; // segundos
    this.gameOver = false;
    this.vidas = data.vidas || 6;
  }

  preload() {
    this.load.image("jugador", "public/assets/secuestrador3.png");
    this.load.image("persona", "public/assets/Sin Título.png");
    this.load.image("casa", "public/assets/Sin Título.png");
    ...
    this.load.image("camion", "public/assets/unrecamion.png");
  }
  ...
  // fondo grande
    ...
    // texto
    this.scoreText = this.add.text(16, 16, "Personas Encerradas: 0 de 5", {
      fontSize: "24px",
      fill: "#ffffff"
    });
    ...
    this.timerText = this.add.text(500, 16, "Tiempo: 20", {
      fontSize: "28px",
      fill: "#ffffff"
    }).setScrollFactor(0);
    ...
    this.camiones = this.physics.add.group();

    // CAMIÓN 1
    let camion1 = this.camiones.create(300, 800, "camion");
    camion1.setScale(0.7);
    camion1.body.setAllowGravity(false);
    camion1.setVelocityX(350); // derecha
    camion1.setCollideWorldBounds(true);
    camion1.setBounce(1);
    camion1.direction = 1; // 1 = derecha, -1 = izquierda

    // CAMIÓN 2
    let camion2 = this.camiones.create(2000, 1500, "camion");
    camion2.setScale(0.7);
    camion2.body.setAllowGravity(false);
    camion2.setVelocityX(-350); // izquierda
    camion2.setCollideWorldBounds(true);
    camion2.setBounce(1);
    camion2.direction = -1;

    this.physics.add.collider(this.camiones, this.paredes, this.rebotarCamion, null, this);
    this.physics.add.collider(this.camiones, this.casitas, this.rebotarCamion, null, this);
  ...
  reachGoal(jugador, casa) {
    if (this.tienePersona) {
      ...
      this.scoreText.setText("Personas Encerradas: " + this.score + " de 5");
      if (this.score >= 5) {
        this.ganar();
      }
    }
  ...
  rebotarCamion(camion) {
    camion.direction *= -1;

    camion.setVelocityX(350 * camion.direction);

    // Cambiar orientación
    if (camion.direction === -1) {
      camion.setFlipX(true);
    } else {
      camion.setFlipX(false);
    }
  }
  golpeCamion(jugador, camion) {
    if (this.gameOver) return;
